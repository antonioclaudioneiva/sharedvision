...
public class PanelClassroom extends JPanel {
	...
	private static final Color MONITOR_FREE_BACKGROUND_COLOR = Color.WHITE;

	private static final Color MONITOR_BUSY_BACKGROUND_COLOR = Color.RED;
	...
	private void drawMonitor(Graphics g, int x, int y, int pos, Integer id) {
		...
		if (ThreadServer.instance.studentsConnecteds.containsKey(id)) {
			g.setColor(MONITOR_BUSY_BACKGROUND_COLOR);
		} else {
			g.setColor(MONITOR_FREE_BACKGROUND_COLOR);
		}
		...
		public void mousePressed(MouseEvent me) {
			...
			for (Integer id : monitors.keySet()) {
				...
				if (me.getX() >= monitor.getX() && me.getX() <= monitor.x + monitor.width && me.getY() >= monitor.y
					...
					System.out.println("id monitor=" + id);
					if (ThreadServer.instance.studentsConnecteds.containsKey(id)) {
						System.out.println("tem o monitor");
						ThreadClient threadClient = ThreadServer.instance.studentsConnecteds.get(id);
						if (threadClient.keyLocked) {
							threadClient.requestUnlockKeys();
						} else {
							threadClient.requestLockKeys();
						}
					}else{
						System.out.println("NÃO tem o monitor");
					}
				}
			}
		}
